...
def basic_exp_utility(env, cost_alloc=0.8, dist_max = 0.5):
    # U(a) = sum of distances sites-users of a linked to sites of a + cost_alloc
    ...
    for i_user in range(n_users):
        ...
        agents_utilities[init_prov] += max_allocated*math.exp(-1.0*dist/dist_max)/(n_SS_u+1) # if init_prov takes dist for him
